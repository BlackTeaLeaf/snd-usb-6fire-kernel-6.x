		...
		unsigned int postaddr, u8 *postdata, unsigned int postlen)
{
	...
	if (ret < 0) {
	     kfree(data);
		kfree(rec);
		printk(KERN_ERR PREFIX "error requesting ezusb "
				"firmware %s.\n", fwname);
		return ret;
	}
	...
	if (ret < 0) {
		...
		kfree(rec);
		release_firmware(fw);
		printk(KERN_ERR PREFIX "error validating ezusb "
				"firmware %s.\n", fwname);
		return ret;
	}
	...
	if (ret < 0) {
		...
		kfree(rec);
		release_firmware(fw);
		printk(KERN_ERR PREFIX "unable to upload ezusb "
		...
		struct usb_interface *intf, const char *fwname)
{
	...
	ret = request_firmware(&fw, fwname, &device->dev);
	if (ret < 0) {
		printk(KERN_ERR PREFIX "unable to get fpga firmware %s.\n",
				fwname);
		kfree(buffer);
		return -EIO;
	}
	...
	if (ret < 0) {
		kfree(buffer);
		release_firmware(fw);
		printk(KERN_ERR PREFIX "unable to upload fpga firmware: "
				"begin urb.\n");
		return ret;
	}
	...
	while (c != end) {
		...
		if (ret < 0) {
			release_firmware(fw);
			kfree(buffer);
			printk(KERN_ERR PREFIX "unable to upload fpga "
					"firmware: fw urb.\n");
			return ret;
		}
	}
...
static int usb6fire_fw_check(u8 *version)
{
	...
	printk(KERN_ERR PREFIX "invalid fimware version in device: "
			"%02x %02x %02x %02x. "
...
int usb6fire_fw_init(struct usb_interface *intf)
{
	...
	if (ret < 0) {
		printk(KERN_ERR PREFIX "unable to receive device "
				"firmware state.\n");
	     kfree(buffer);
		return ret;
	}
	...
	if (buffer[0] != 0xeb || buffer[1] != 0xaa || buffer[2] != 0x55) {
		printk(KERN_ERR PREFIX "unknown device firmware state "
